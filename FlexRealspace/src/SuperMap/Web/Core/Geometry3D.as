package SuperMap.Web.Core
{
	...
	public class Geometry3D extends Geometry
	{
		 ...
		 * 如果与当前几何对象有交集返回 true；否则返回 false。
		 */
		...
		/**
		 * 按指定比例缩放组成几何对象的坐标值。
		 * dRatioX ：X 方向（经度方向）缩放比率。
		 * dRatioY ：Y 方向（纬度方向）缩放比率。
		 * dRatioZ ：Z 方向（高度）缩放比率。
		 */
		 ...
		 * offsetX ：偏移 X 坐标（经度方向）的量，单位为度。 
		 * offsetY ：偏移 Y 坐标（纬度方向）的量，单位为度。
		 */
